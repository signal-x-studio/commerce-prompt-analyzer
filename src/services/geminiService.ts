import { Engine, GenerationResult, TestResult, UsageMetadata } from "../types";

export async function generateCatalogStructure(
  url: string,
  mock: boolean = false,
...
export async function generateCustomerPrompts(
  structure: any,
  mock: boolean = false,
  trackUsage?: (usage?: UsageMetadata) => void
...
export async function generatePromptsFromUrl(
  url: string,
...
export async function testPromptAnswerability(
  prompt: string,
  userUrl: string,
  engine: Engine,
  mock: boolean = false,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`API Error testing prompt: ${errorMessage}`);
    ...
    console.error(
      `Error testing prompt "${prompt}" with engine "${engine.id}":`,
      error
    );
    return { status: "error" };
  }
